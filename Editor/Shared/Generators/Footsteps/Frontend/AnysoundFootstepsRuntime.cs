...
{
    //todo - this class shares a lot of code with AnysoundFootstepsEditorWindow. - code should be moved to a central place..
    public class AnysoundFootstepsRuntime : MonoBehaviour
    {
        ...
        void GeneratePreview()
        {
            ...
            if (previewClip != null)
            {
                _waveformContainer.style.backgroundImage = new StyleBackground(WaveformMaker.GenerateWaveformTexture(previewClip));
            }
        }
        ...
        void UpdateMovementVisuals()
        {
            ...
            _movementLabelsContainer.Query<Label>().ForEach((element =>
            {
                if (element != _sizeIconsContainer)
                {
                    ...
                    element.style.color = (index == _currentMovementIndex
                        ? Color.black
                        : AnysoundFootstepsHelper.AnysoundFootstepsEditorExtensions.HexToColor("60C75C"));
                    index++;
                }
            }));
            UpdateSizeImages();
